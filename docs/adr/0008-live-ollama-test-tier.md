...
**Route this through sentinel-eval's online pipeline instead of an in-repo test.** Rejected as a *replacement* — different purpose and cadence. sentinel-eval's online pipeline is for continuous precision/recall/F1 measurement across many fixtures against the judge-availability circuit breaker; this ADR is a local smoke check that the extractor's LLM path is wired correctly at all. sentinel-eval consuming Synapse-L4's extractor as an eval subject later (planned) is complementary, not a substitute.
